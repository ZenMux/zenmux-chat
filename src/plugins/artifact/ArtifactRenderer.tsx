...
const ArtifactRenderer = memo<ArtifactCardProps>((props) => {
  ...
  return (
    <button
      onClick={handleClick}
      className="my-2 w-full flex items-center gap-3 px-4 py-3 rounded-lg border border-chat-border bg-chat-bg hover:bg-chat-sidebar-bg hover:border-chat-border shadow-sm transition-all duration-150 cursor-pointer text-left group"
    >
      {/* 图标 */}
      <div className="w-9 h-9 rounded-lg bg-chat-hover group-hover:bg-violet-50 flex items-center justify-center text-chat-text-secondary group-hover:text-violet-600 transition-colors shrink-0">
      ...
      {/* 箭头 */}
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" className="text-chat-text-muted group-hover:text-chat-text-secondary transition-colors shrink-0">
